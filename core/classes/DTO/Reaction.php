...
class Reaction
{
    ...
    {
        $rows = DB::getInstance()->query('SELECT * FROM rw_reactions ORDER BY `order`')->results();
        $fields = [];
        foreach ($rows as $row) {
    ...
    {
        $rows = DB::getInstance()->query("SELECT * FROM rw_reactions WHERE `$column` = ? ORDER BY `order`", [$value]);
        if (!$rows->count()) {
            return [];
        }
