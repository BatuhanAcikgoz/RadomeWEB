...
class Placeholders extends Instanceable {
    ...
    public function __construct() {
        ...
        if (isset($_GET['action'])) {
            ...
            case 'delete':
                if (Token::check()) {
                    if (isset($data->name)) {
                        DB::getInstance()->delete('users_placeholders', ['name', $_GET['id']]);
                        DB::getInstance()->delete('placeholders_settings', ['name', $_GET['id']]);
    ...
    /**
     * Create a new row in rw_placeholders_settings if a row with the "server_id" of $server_id and "name" of $name does not exist (this lets the same placeholder name be used across multiple RadomeWEB plugin servers).
     *
    ...
    public function getLeaderboardData(int $server_id, string $placeholder_name): array {
        ...
        $leaderboard_data = $this->_db->query("SELECT * FROM rw_users_placeholders WHERE name = ? AND server_id = ? ORDER BY REPLACE(value, ',', '') + 0 {$sort} LIMIT 50", [$placeholder_name, $server_id]);
