...
class IntegrationUser {
    ...
    public function linkIntegration(User $user, ?string $identifier, ?string $username, bool $verified = false, string $code = null): void {
        ...
        $rw_int_id = $this->_integration->data()->id;
        $minecraft_int = DB::getInstance()->query("SELECT integration_id FROM rw_users_integrations WHERE user_id = {$rw_int_id}")->results();
        if ($minecraft_int === 1) {
