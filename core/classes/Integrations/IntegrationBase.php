...
abstract class IntegrationBase {
    ...
    public function __construct() {
        ...
        $integration = $this->_db->query('SELECT * FROM rw_integrations WHERE name = ?', [$this->_name]);
        if ($integration->count()) {
            ...
            $this->_db->query('INSERT INTO rw_integrations (name) VALUES (?)', [
                $this->_name
            ]);

            $integration = $this->_db->query('SELECT * FROM rw_integrations WHERE name = ?', [$this->_name])->first();
