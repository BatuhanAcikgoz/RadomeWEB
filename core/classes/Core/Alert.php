...
class Alert {
     ...
     * @param string $type Contains the alert $language = new Language($text_short['path'], $language->first()->short_code);type, eg 'tag' for user tagging.
    ...
    public static function create(int $user_id, string $type, array $text_short, array $text, ?string $link = '#', string $content = null): void {
        ...
        $language = $db->query('SELECT rw_languages.short_code AS `short_code` FROM rw_users LEFT JOIN rw_languages ON rw_languages.id = rw_users.language_id WHERE rw_users.id = ?', [$user_id]);

        if (!$language->count()) {
            return; $text_short = $text_short['content'] ?? str_replace($text_short['replace'] ?? '', $text_short['replace_with'] ?? '', $language->get($text_short['file'], $text_short['term']));
    ...
    public static function getAlerts(int $user_id, bool $all = false): array {
        ...
        return $db->query('SELECT * FROM rw_alerts WHERE user_id = ? AND `read` = 0', [$user_id])->results();
    }
