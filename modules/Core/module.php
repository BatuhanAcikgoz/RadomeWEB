...
class Core_Module extends Module {
    ...
    public function onPageLoad(User $user, Pages $pages, Cache $cache, Smarty $smarty, $navs, Widgets $widgets, ?TemplateBase $template) {
        ...
        PermissionHandler::registerPermissions($language->get('moderator', 'staff_cp'), [
            ...
            'admincp.minecraft.tier_lisr' => $language->get('admin', 'integrations') . ' &raquo; ' . $language->get('admin', 'minecraft') . ' &raquo; ' . $language->get('admin', 'tier_list'),
        ...
        if (Settings::get('tier_list_page') === '1') {
            ...
            if (!$cache->isCached('tier_list_icon')) {
                $tier_list_icon = '<i class="fas fa-layer-group"></i>';
            } else {
                $tier_list_icon = $cache->retrieve('tier_list_icon');
            }

            $navs[0]->addItemToDropdown('more_dropdown', 'tier_list', $language->get('admin', 'tier_list'), URL::build('/tier_list'), 'top', null, $tier_list_icon, $tier_list_order);
        }
        ...
        if (defined('FRONT_END')) {
            ...
            if ($user->hasPermission('admincp.core.emails')) {
                if ($cache->isCached('email_errors')) {
                    ...
                    $email_errors = DB::getInstance()->query('SELECT COUNT(*) AS c FROM rw_email_errors')->first()->c;
            ...
            if ($user->hasPermission('admincp.users.sessions')) {
                self::addUserAction($language->get('general', 'sessions'), URL::build('/panel/kullanici/sessions', 'id={id}'));
            }
