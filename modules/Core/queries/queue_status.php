...
$total = $db->query('SELECT COUNT(*) as `total` FROM rw_queue', [])->first()->total;
$query = 'SELECT `id`, `name`, `scheduled_for`, `status`, `task` FROM rw_queue';
$where = '';
$order = '';
$limit = '';
$params = [];
...
if (strlen($where) > 0) {
    $totalFiltered = $db->query('SELECT COUNT(*) as `total` FROM rw_queue' . $where, $params)->first()->total;
}
