...
if (isset($_GET['group'])) {
    $extra_query .= ' INNER JOIN rw_users_groups ug ON u.id = ug.user_id';
    $where[] = 'ug.group_id = ?';
    $params[] = $_GET['group'];
}

if (isset($_GET['integration'])) {
    $extra_query .= ' INNER JOIN rw_users_integrations ui ON ui.user_id=u.id INNER JOIN rw_integrations i ON i.id=ui.integration_id';
    $where[] = 'i.name = ?';
...
if (strlen($where) > 0) {
    $totalFiltered = $db->query('SELECT COUNT(*) as `total` FROM rw_users u ' . $extra_query . $where, $params)->first()->total;
}
