                ...
                foreach ($files as $file) {
                    if ($file != '.' && $file != '..' && (is_dir($template_path . DIRECTORY_SEPARATOR . $file) || pathinfo($file, PATHINFO_EXTENSION) == 'tpl' || pathinfo($file, PATHINFO_EXTENSION) == 'css' || pathinfo($file, PATHINFO_EXTENSION) == 'js' || pathinfo($file, PATHINFO_EXTENSION) == 'conf')) {
                        if (!is_dir($template_path . DIRECTORY_SEPARATOR . $file)) {
                            $template_files[] = [
                                'link' => URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id) . '&file=' . urlencode($file)),
                                'name' => Output::getClean($file)
                            ];
                        } else {
                            $template_dirs[] = [
                                'link' => URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id) . '&dir=' . urlencode($file)),
                                'name' => Output::getClean($file)
                            ];
                        }
                    }
                ...
                if (isset($_GET['dir']) && !isset($_GET['file'])) {
                    ...
                    foreach ($files as $file) {
                        if ($file != '.' && $file != '..' && (is_dir($template_path . DIRECTORY_SEPARATOR . $file) || pathinfo($file, PATHINFO_EXTENSION) == 'tpl' || pathinfo($file, PATHINFO_EXTENSION) == 'css' || pathinfo($file, PATHINFO_EXTENSION) == 'js' || pathinfo($file, PATHINFO_EXTENSION) == 'conf')) {
                            if (!is_dir($template_path . DIRECTORY_SEPARATOR . $file)) {
                                $template_files[] = [
                                    'link' => URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id) . '&dir=' . urlencode($dir) . '&file=' . urlencode($file)),
                                    'name' => Output::getClean($file)
                                ];
                            } else {
                                $template_dirs[] = [
                                    'link' => URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id) . '&dir=' . urlencode($dir) . DIRECTORY_SEPARATOR . urlencode($file)),
                                    'name' => Output::getClean($file)
                                ];
                            }
                        }
                    ...
                    if (count($dirs) > 1) {
                        ...
                        $back_link = URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id) . '&dir=' . urlencode($new_dir));
                    } else {
                        $back_link = URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($template_query->id));
                    }
                    ...
                    $smarty->assign([
                        ...
                        'EDIT' => $language->get('general', 'edit'),
                        'PERMISSIONS' => $language->get('admin', 'permissions'),
                        'PERMISSIONS_LINK' => $user->hasPermission('admincp.groups') ? '/panel/temalar/?template=' . Output::getClean($template_query->id) . '&action=permissions' : null,
                    ]);
                        ...
                        if (Input::exists()) {
                            if (Token::check()) {
                                ...
                                if (is_writable($file_path)) {
                                    ...
                                    if (isset($_GET['dir'])) {
                                        Redirect::to(URL::build('/panel/temalar/', 'action=edit&template=' . $_GET['template'] . '&dir=' . urlencode($_GET['dir']) . '&file=' . urlencode($_GET['file'])));
                                    } else {
                                        Redirect::to(URL::build('/panel/temalar/', 'action=edit&template=' . $_GET['template'] . '&file=' . urlencode($_GET['file'])));
                                    }
                                }
...
// No write permission
                        ...
                        if (isset($_GET['dir'])) {
                            $cancel_link = URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($_GET['template']) . '&dir=' . urlencode($_GET['dir']));
                        } else {
                            $cancel_link = URL::build('/panel/temalar/', 'action=edit&template=' . urlencode($_GET['template']));
                        }
