...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'new':
                ...
                if (Token::check()) {
                    ...
                    if ($validation->passed()) {
                        try {
                            ...
                            Redirect::to(URL::build('/panel/gruplar'));
                        } catch (Exception $e) {
                            $errors[] = $e->getMessage();
                        }
                    } else {
            ...
            $smarty->assign([
                ...
                'CANCEL_LINK' => URL::build('/panel/gruplar'),
        ...
        case 'edit':
            ...
            if ($group->id == 2 || ((in_array($group->id, $user->getAllGroupIds())) && !$user->hasPermission('admincp.groups.self'))) {
                ...
                $smarty->assign([
                    ...
                    'PERMISSIONS_LINK' => URL::build('/panel/gruplar/', 'action=permissions&group=' . urlencode($group->id)),
                    ...
                    if (Input::get('action') == 'update') {
                        ...
                        if ($validation->passed()) {
                            try {
                                ...
                                Redirect::to(URL::build('/panel/gruplar/', 'action=edit&group=' . urlencode($_GET['group'])));
                            } catch (Exception $e) {
                                $errors[] = $e->getMessage();
                            }
                        } else {
                    ...
                    if (Input::get('action') == 'update') {
                        ...
                        if ($validation->passed()) {
                            try {
                                ...
                                Redirect::to(URL::build('/panel/gruplar/', 'action=edit&group=' . urlencode($group_id)));
                            } catch (Exception $e) {
                                $errors[] = $e->getMessage();
                            }
                        } else {
        ...
        case 'permissions':
            ...
            if (Input::exists()) {
                ...
                if (Token::check()) {
                    ...
                    try {
                        ...
                        Redirect::to(URL::build('/panel/gruplar/', 'action=edit&group=' . urlencode($group->id)));
                    } catch (Exception $e) {
                        $errors[] = $e->getMessage();
                    }
                } else {
            ...
            $smarty->assign([
                ...
                'BACK_LINK' => URL::build('/panel/gruplar/', 'action=edit&group=' . urlencode($group->id)),
    ...
    foreach (Group::all() as $group) {
        $groups_template[] = [
            ...
            'us     * @param string $link       Contains link to view the alert, defaults to #.ers_link' => URL::build('/panel/users/', 'group=' . $group->id),
            'staff' => $group->staff
        ];
    }
    ...
    $smarty->assign([
        ...
        'NEW_GROUP_LINK' => URL::build('/panel/gruplar/', 'action=new'),
        ...
        'REORDER_DRAG_URL' => URL::build('/panel/gruplar', 'action=order')
    ]);
