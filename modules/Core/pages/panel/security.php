                ...
                $rows[] = [
                    0 => [
                        'content' => $target_user->exists()
                            ? '<a style="' . $target_user->getGroupStyle() . '" href="' . URL::build('/panel/kullanici/' . urlencode($log->user_id . '-' . $target_user->getDisplayname(true))) . '">' . Output::getClean($target_user->getDisplayname()) . '</a>'
                ...
                $rows[] = [
                    0 => [
                        'content' => $log->user_id == 0
                            ...
                            : ($target_user->exists() ? '<a style="' . $target_user->getGroupStyle() . '" href="' . URL::build('/panel/kullanici/' . urlencode($log->user_id . '-' . $target_user->getDisplayname(true))) . '">' . Output::getClean($target_user->getDisplayname()) . '</a>'
                            : $language->get('general', 'deleted_user'))
                    ],
                    1 => [
                        'content' => '<a href="' . URL::build('/panel/kullanicilar/ip_sorgu/', 'ip=' . urlencode($log->ip)) . '">' . Output::getClean($log->ip) . '</a>'
