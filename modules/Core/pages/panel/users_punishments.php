                    ...
                    try {
                        ...
                        if (!$is_admin) {
                            ...
                            if ($type != 3 || $user->data()->lastip != $banned_user->data()->lastip) {
                                ...
                                // Send alerts
                                $groups_query = DB::getInstance()->query('SELECT id FROM rw_groups WHERE permissions LIKE \'%"modcp.punishments":1%\'');
                                if ($groups_query->count()) {
                                    ...
                                    $users = DB::getInstance()->query('SELECT DISTINCT(rw_users.id) AS id FROM rw_users LEFT JOIN rw_users_groups ON rw_users.id = rw_users_groups.user_id WHERE group_id in ' . $groups);
