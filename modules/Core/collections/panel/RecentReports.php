...
class RecentReportsItem extends CollectionItemBase {
    ...
    public function getContent(): string {
        ...
        if ($this->_cache->isCached('recent_reports_data')) {
            ...
            $query = DB::getInstance()->query('SELECT * FROM rw_reports WHERE `status` = 0 ORDER BY `date_reported` DESC LIMIT 5');
            $data = [];
            ...
            if ($query->count()) {
                ...
                foreach ($query->results() as $item) {
                    ...
                    $data[] = [
                        ...
                        'reporter_profile' => URL::build('/panel/kullanici/' . urlencode($reporter_user->data()->id) . '-' . urlencode($reporter_user->data()->username)),
                        ...
                        'reported_profile' => URL::build('/panel/kullanici/' . urlencode($reported_user->data()->id) . '-' . urlencode($reported_user->data()->username)),
