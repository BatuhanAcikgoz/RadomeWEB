...
class ServerStatusWidget extends WidgetBase {
    ...
    public function initialise(): void {
        ...
        if ($this->_cache->isCached('server_status')) {
            ...
            $server = DB::getInstance()->query('SELECT * FROM rw_mc_servers WHERE is_default = 1')->results();
            $server = $server[0];

            if ($server != null) {
                $server_array_request = HttpClient::get(rtrim(URL::getSelfURL(), '/') . URL::build('/sorgu/sunucu/', 'id=' . $server->id));
