...
class StaffMembersListProvider extends MemberListProvider {
    ...
    protected function generator(): array {
        return [
            'SELECT u.id FROM rw_users u INNER JOIN rw_users_groups ug ON u.id = ug.user_id INNER JOIN rw_groups g ON ug.group_id = g.id WHERE g.staff = 1 GROUP BY u.username, u.id ORDER BY u.username',
            'id',
        ];
    }
}