...
class DiscordIntegration extends IntegrationBase {
    ...
    public function onUnlinkRequest(User $user) {
        ...
        $userint = DB::getInstance()->get('users_integrations', ['user_id', '=', $user->data()->id, 'and', 'integration_id', '=', 2])->results();
    ...
    public function validateUsername(string $username, int $integration_user_id = 0): bool {
        ...
        if (count($validation->errors())) {
            ...
            $exists = DB::getInstance()->query("SELECT * FROM rw_users_integrations WHERE integration_id = ? AND username = ? AND id <> ?", [$this->data()->id, $username, $integration_user_id]);
            if ($exists->count()) {
    ...
    public function validateIdentifier(string $identifier, int $integration_user_id = 0): bool {
        ...
        if (count($validation->errors())) {
            ...
            // Ensure identifier doesn't already exist
            $exists = DB::getInstance()->query("SELECT * FROM rw_users_integrations WHERE integration_id = ? AND identifier = ? AND id <> ?", [$this->data()->id, $identifier, $integration_user_id]);
            if ($exists->count()) {
