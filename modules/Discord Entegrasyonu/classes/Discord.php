 ...
 *
 * @package Modules\Discord Entegrasyonu
 * @author Aberdeener
 * @version 2.0.0-pr13
 * @license MIT
 */
class Discord {
    ...
    private static function assembleJson(int $user_id, array $change_arr): string {
        ...
        return json_encode([
            ...
            'user_id' => $user_id,
            'api_key' => trim(Settings::get('mc_api_key')),
            'roles' => $change_arr,
        ]);
    }
    ...
    /**
     * Get a language term for the Discord Entegrasyonu module.
     *
    ...
    public static function getLanguageTerm(string $term, array $variables = []): string {
        if (!isset(self::$_discord_integration_language)) {
            self::$_discord_integration_language = new Language(ROOT_PATH . '/modules/Discord Entegrasyonu/language');
        }
