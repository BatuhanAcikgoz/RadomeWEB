    ...
    public static function globalLimit(array $params = []): array {
        ...
        if (isset($global_limit['limit']) && $global_limit['limit'] > 0) {
            ...
            if (isset($global_limit['period']) && $global_limit['period'] != 'no_period' && isset($global_limit['interval']) && $global_limit['interval'] > 0) {
                $limit = DB::getInstance()->query('SELECT count(*) as c FROM rw_forms_replies WHERE form_id = ? AND created > ?', [$form->data()->id, strtotime('-'.$global_limit['interval'].' ' . $global_limit['period'])]);
            } else {
                $limit = DB::getInstance()->query('SELECT count(*) as c FROM rw_forms_replies WHERE form_id = ?', [$form->data()->id]);
            }

            if ($limit->first()->c >= $global_limit['limit']) {
                $params['errors'][] = Formlar::getLanguage()->get('forms', 'form_global_limit_reached');
            }
        }
        ...
        if ($user->isLoggedIn()) {
            ...
            if (isset($user_limit['limit']) && $user_limit['limit'] > 0) {
                ...
                if (isset($user_limit['period']) && $user_limit['period'] != 'no_period' && isset($user_limit['interval']) && $user_limit['interval'] > 0) {
                    $limit = DB::getInstance()->query('SELECT count(*) as c FROM rw_forms_replies WHERE form_id = ? AND user_id = ? AND created > ?', [$form->data()->id, $user->data()->id, strtotime('-'.$user_limit['interval'].' ' . $user_limit['period'])]);
                } else {
                    $limit = DB::getInstance()->query('SELECT count(*) as c FROM rw_forms_replies WHERE form_id = ? AND user_id = ?', [$form->data()->id, $user->data()->id]);
                }

                if ($limit->first()->c >= $user_limit['limit']) {
                    $params['errors'][] = Formlar::getLanguage()->get('forms', 'form_user_limit_reached');
                }
            }
        }
        ...
        if ($user->isLoggedIn()) {
            ...
            foreach ($required_integrations_list as $integration) {
                ...
                if ($integrationUser == null || $integrationUser->data()->username == null || $integrationUser->data()->identifier == null) {
                    $params['errors'][] = Formlar::getLanguage()->get('forms', 'form_requires_integration', [
