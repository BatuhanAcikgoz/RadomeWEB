...
class Formlar_Module extends Module {
    ...
    public function __construct($language, $forms_language, $pages, $user, $navigation, $cache, $endpoints) {
        ...
        $pages->add('Formlar', '/panel/form', 'pages/panel/form.php');
        $pages->add('Formlar', '/panel/formlar', 'pages/panel/forms.php');
        $pages->add('Formlar', '/panel/formlar/durumlar', 'pages/panel/statuses.php');
        $pages->add('Formlar', '/panel/formlar/talepler', 'pages/panel/submissions.php');
        $pages->add('Formlar', '/kullanici/talepler', 'pages/user/submissions.php');
        ...
        try {
            ...
            if (count($forms)) {
                ...
                foreach ($forms as $form) {
                    // Register form page
                    $pages->add('Formlar', $form->url, 'pages/form.php', 'form-' . $form->id, true);
        ...
        Endpoints::registerTransformer('form', 'Formlar', static function (Radome2API $api, string $value) {
            if (is_numeric($value)) {
                // Get form by id
                $form = new Form($value);
            } else {
            ...
            $api->throwError(FormlarApiErrors::ERROR_FORM_NOT_FOUND);
        });

        Endpoints::registerTransformer('submission', 'Formlar', static function (Radome2API $api, string $value) {
            $submission = new Submission($value);
            if ($submission->exists()) {
                return $submission;
            }

            $api->throwError(FormlarApiErrors::ERROR_SUBMISSION_NOT_FOUND);
        });
    }
    ...
    public function onPageLoad($user, $pages, $cache, $smarty, $navs, $widgets, $template) {
        // Permissions
        PermissionHandler::registerPermissions('Formlar', array(
