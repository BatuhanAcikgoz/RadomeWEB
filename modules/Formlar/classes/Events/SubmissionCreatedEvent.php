...
class SubmissionCreatedEvent extends AbstractEvent implements HasWebhookParams, DiscordDispatchable {
    ...
    public static function description(): string {
        return (new Language(ROOT_PATH . '/modules/Formlar/language'))->get('forms', 'new_form_submission');
    }
    ...
    public function toDiscordWebhook(): DiscordWebhookBuilder {
        $language = new Language(ROOT_PATH . '/modules/Formlar/language', DEFAULT_LANGUAGE);

        return DiscordWebhookBuilder::make()
            ->setUsername(($this->user != null && $this->user->exists() ? $this->user->getDisplayname() : Formlar::getLanguage()->get('forms', 'guest')) . ' | ' . SITE_NAME)
            ...
            ->addEmbed(function (DiscordEmbed $embed) use ($language) {
                $embed
                    ...
                    ->setFooter($language->get('forms', 'new_submission_in_form', [
                        'form' => $this->form->data()->title,
                        'user' => ($this->user != null && $this->user->exists() ? $this->user->getDisplayname() : Formlar::getLanguage()->get('forms', 'guest'))
                    ]))
