...
class ConvertForumPostTask extends Task {
    ...
    public static function schedule() {
        $hasBeenScheduled = DB::getInstance()->query('SELECT COUNT(*) c FROM rw_queue WHERE `task` = \'ConvertForumPostTask\'')->first()->c;

        if (!$hasBeenScheduled) {
            $totalForumPosts = DB::getInstance()->query('SELECT COUNT(*) c FROM rw_posts')->first()->c;
