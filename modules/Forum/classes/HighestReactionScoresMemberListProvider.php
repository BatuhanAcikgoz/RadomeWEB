...
class HighestReactionScoresMemberListProvider extends MemberListProvider {
    ...
    protected function generator(): array {
        return [
            'SELECT COUNT(fr.user_received) AS `count`, fr.user_received FROM rw_forums_reactions fr JOIN rw_reactions r ON r.id = fr.reaction_id WHERE r.type = 2 GROUP BY fr.user_received ORDER BY `count` DESC',
            'user_received',
            'count'
        ];
    }
