...
class Forum {
                    ...
                    if (count($forums)) {
                        foreach ($forums as $item) {
                            if ($this->forumExist($item->id, $groups)) {
                                ...
                                $return[$forum->id]['subforums'][$item->id]->link = URL::build('/forum/bakis/' . urlencode($item->id) . '-' . $this->titleToURL($item->forum_title));
                                ...
                                if (isset($last_reply) && count($last_reply)) {
                                    ...
                                    $return[$forum->id]['subforums'][$item->id]->last_post->link = URL::build('/forum/konu/' . urlencode($last_reply[$n]->topic_id) . '-' . $this->titleToURL($last_topic[0]->topic_title), 'pid=' . $last_reply[0]->id);
                                }
                                ...
                                if (count($subforums)) {
                                    foreach ($subforums as $subforum) {
                                        if ($this->forumExist($subforum->id, $groups)) {
                                            ...
                                            $return[$forum->id]['subforums'][$item->id]->subforums[$subforum->id]->link = URL::build('/forum/bakis/' . urlencode($subforum->id) . '-' . $this->titleToURL($subforum->forum_title));
    ...
    public function getLatestDiscussions(array $groups = [0], int $user_id = 0): array {
        ...
        $all_topics_forums = DB::getInstance()->query('SELECT forum_id FROM rw_forums_permissions WHERE group_id IN (' . rtrim(implode(',', $groups), ',') . ') AND `view` = 1 AND view_other_topics = 1')->results();

        $own_topics_forums = [];
        if ($user_id > 0) {
            $own_topics_forums = DB::getInstance()->query('SELECT forum_id FROM rw_forums_permissions WHERE group_id IN (' . rtrim(implode(',', $groups), ',') . ') AND `view` = 1 AND view_other_topics = 0')->results();
        }
        ...
        if (count($own_topics_forums)) {
            ...
            return DB::getInstance()->query(
                '(SELECT topics.id as id, topics.forum_id as forum_id, topics.topic_title as topic_title, topics.topic_creator as topic_creator, topics.topic_last_user as topic_last_user, topics.topic_date as topic_date, topics.topic_reply_date as topic_reply_date, topics.topic_views as topic_views, topics.locked as locked, topics.sticky as sticky, topics.label as label, topics.deleted as deleted, posts.id as last_post_id FROM rw_topics topics LEFT JOIN rw_posts posts ON topics.id = posts.topic_id AND posts.id = (SELECT MAX(id) FROM rw_posts p WHERE p.topic_id = topics.id AND p.deleted = 0) WHERE topics.deleted = 0 AND topics.forum_id IN (' . $all_topics_forums_string . ') ORDER BY topics.topic_reply_date DESC LIMIT 50)
                UNION
                (SELECT topics.id as id, topics.forum_id as forum_id, topics.topic_title as topic_title, topics.topic_creator as topic_creator, topics.topic_last_user as topic_last_user, topics.topic_date as topic_date, topics.topic_reply_date as topic_reply_date, topics.topic_views as topic_views, topics.locked as locked, topics.sticky as sticky, topics.label as label, topics.deleted as deleted, posts.id as last_post_id FROM rw_topics topics LEFT JOIN rw_posts posts ON topics.id = posts.topic_id AND posts.id = (SELECT MAX(id) FROM rw_posts p WHERE p.topic_id = topics.id AND p.deleted = 0) WHERE topics.deleted = 0 AND ((topics.forum_id IN (' . $own_topics_forums_string . ') AND topics.topic_creator = ?) OR topics.sticky = 1) ORDER BY topics.topic_reply_date DESC LIMIT 50)
        ...
        return DB::getInstance()->query(
            ...
            FROM rw_topics topics 
            LEFT JOIN rw_posts posts ON topics.id = posts.topic_id AND posts.id = (SELECT MAX(id) FROM rw_posts p WHERE p.topic_id = topics.id AND p.deleted = 0) 
    ...
    public function getLatestNews(int $number = 5): array {
        ...
        $news_items = $this->_db->query('SELECT * FROM rw_topics WHERE forum_id IN (SELECT id FROM rw_forums WHERE news = 1) AND deleted = 0 ORDER BY topic_date DESC LIMIT 10')->results();
    ...
    public function getAnySubforums(int $forum_id, array $groups = [0], int $depth = 0): array {
        ...
        $subforums_query = $this->_db->query('SELECT * FROM rw_forums WHERE parent = ? ORDER BY forum_order ASC', [$forum_id]);
