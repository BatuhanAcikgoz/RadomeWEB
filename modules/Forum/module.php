...
class Forum_Module extends Module {
            ...
            return [
                $forum_language->get('forum', 'posts_title') =>
                    DB::getInstance()->query(
                        'SELECT COUNT(post_content) AS `count` FROM rw_posts WHERE post_creator = ?',
                        [$member->data()->id]
                    )->first()->count,
            ];
        });
        ...
        MemberListManager::getInstance()->registerMemberMetadataProvider(function (User $member) use ($forum_language) {
            return [
                $forum_language->get('forum', 'reaction_score') =>
                    DB::getInstance()->query(
                        'SELECT COUNT(fr.user_received) AS `count` FROM rw_forums_reactions fr JOIN rw_reactions r ON r.id = fr.reaction_id WHERE r.type = 2 AND fr.user_received = ?',
                        [$member->data()->id]
                    )->first()->count,
            ];
        });
    ...
    public function onPageLoad($user, $pages, $cache, $smarty, $navs, $widgets, $template) {
        ...
        if (defined('FRONT_END') || (defined('PANEL_PAGE') && str_contains(PANEL_PAGE, 'widget'))) {
            // Latest posts
            $widgets->add(new LatestPostsWidget($this->_forum_language->get('forum', 'latest_submissions'), $this->_forum_language->get('forum', 'by'), $smarty, $cache, $user, $this->_language));
        }
        ...
        if (defined('FRONT_END')) {
            ...
            if (defined('BACK_END')) {
                ...
                if (defined('PANEL_PAGE') && PANEL_PAGE == 'dashboard') {
                    ...
                    $latest_submissions = DB::getInstance()->query(
                        <<<SQL
                    ...
                    if ($cache->isCached('forum_data')) {
                        ...
                        $data['datasets']['submissions']['label'] = 'forms_language/forms/submissions'; // for $forum_language->get('forum', 'posts_title');
                        $data['datasets']['submissions']['colour'] = '#ff7f00';
                        ...
                        if (count($latest_submissions)) {
                            foreach ($latest_submissions as $day) {
                                if (isset($data['_' . $day->d])) {
                                    $data['_' . $day->d]['submissions'] = $day->c;
                                } else {
                                    $data['_' . $day->d] = ['submissions' => $day->c];
                                }
                            }
                        ...
                        $data = Core_Module::fillMissingGraphDays($data, 'submissions');
