        ...
        if (Token::check()) {
            ...
            if ($validation->passed()) {
                ...
                DB::getInstance()->query(
                    "UPDATE rw_posts SET `forum_id` = ? WHERE `id` IN ($posts)",
                ...
                Redirect::to(URL::build('/forum/konu/' . $topic_id));
...
foreach ($categories as $category) {
    ...
    $forums = DB::getInstance()->query('SELECT * FROM rw_forums WHERE parent = ? ORDER BY forum_order ASC', [$category->id]);
...
$smarty->assign([
    ...
    'CANCEL_LINK' => URL::build('/forum/konu/' . urlencode($topic->id)),
