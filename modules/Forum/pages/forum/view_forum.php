...
if (isset($_GET['p'])) {
    ...
    if ($_GET['p'] == 1) {
        ...
        Redirect::to(URL::build('/forum/bakis/' . urlencode($fid) . '-' . $forum->titleToURL($forum_query->forum_title)));
    }
    $p = $_GET['p'];
} else {
    $p = 1;
...
$page_metadata = DB::getInstance()->get('page_descriptions', ['page', '/forum/bakis'])->results();
...
if ($forum_query->redirect_forum == 1) {
    ...
    if ($forum->canViewOtherTopics($fid, $user_groups)) {
        $topics = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND sticky = 0 AND deleted = 0 ORDER BY topic_reply_date DESC', [$fid])->results();
    } else {
        $topics = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND sticky = 0 AND deleted = 0 AND topic_creator = ? ORDER BY topic_reply_date DESC', [$fid, $user_id])->results();
    }

    // Get sticky topics
    $stickies = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND sticky = 1 AND deleted = 0 ORDER BY topic_reply_date DESC', [$fid])->results();

    // Search bar
    $smarty->assign([
        'SEARCH_URL' => URL::build('/forum/arama'),
    ...
    $breadcrumbs = [0 => [
        ...
        'active' => 1,
        'link' => URL::build('/forum/bakis/' . urlencode($forum_query->id) . '-' . $forum->titleToURL($forum_query->forum_title))
    ]];
        ...
        foreach ($subforums as $subforum) {
            ...
            if ($forum->forumExist($subforum->id, $user_groups)) {
                if ($forum->canViewOtherTopics($subforum->id, $user_groups)) {
                    $latest_post = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND deleted = 0 ORDER BY topic_reply_date DESC', [$subforum->id])->results();
                } else {
                    $latest_post = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND deleted = 0 AND (topic_creator = ? OR sticky = 1) ORDER BY topic_reply_date DESC', [$subforum->id, $user_id])->results();
                }
                ...
                if (count($latest_post)) {
                    ...
                    $latest_post_link = URL::build('/forum/konu/' . urlencode($latest_post->id) . '-' . $forum->titleToURL($latest_post->topic_title));
                ...
                $subforum_array[] = [
                    ...
                    'link' => URL::build('/forum/bakis/' . urlencode($subforum->id) . '-' . $forum->titleToURL($subforum->forum_title)),
    ...
    if ($user->isLoggedIn() && $forum->canPostTopic($fid, $user_groups)) {
        $smarty->assign('NEW_TOPIC_BUTTON', URL::build('/forum/yeni/', 'fid=' . urlencode($fid)));
    } else {
    ...
    if (!count($stickies) && !count($topics)) {
        ...
        foreach ($stickies as $sticky) {
            ...
            $sticky_array[] = [
                ...
                'link' => URL::build('/forum/konu/' . urlencode($sticky->id) . '-' . $forum->titleToURL($sticky->topic_title)),
        ...
        $pagination = $paginator->generate(7, URL::build('/forum/bakis/' . urlencode($fid) . '-' . $forum->titleToURL($forum_query->forum_title)));
        ...
        foreach ($results->data as $nValue) {
            ...
            $template_array[] = [
                ...
                'labels' => $labels,
                'author_link' => $topic_user->getProfileURL(),
                'link' => URL::build('/forum/konu/' . urlencode($nValue->id) . '-' . $forum->titleToURL($nValue->topic_title)),
                'last_reply_link' => $last_reply_user->getProfileURL(),
