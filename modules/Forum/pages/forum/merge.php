...
$topic_id = $_GET['tid'];
$forum_id = DB::getInstance()->query('SELECT forum_id FROM rw_topics WHERE id = ?', [$topic_id])->first();
        ...
        if (Token::check()) {
            ...
            if ($validation->passed()) {
                ...
                DB::getInstance()->query(
                    "UPDATE rw_posts SET `topic_id` = ? WHERE `id` IN ($posts)",
                    [Input::get('merge')]
                );
                ...
                Redirect::to(URL::build('/forum/konu/' . urlencode(Input::get('merge'))));
...
// Get topics
$topics = DB::getInstance()->query('SELECT * FROM rw_topics WHERE forum_id = ? AND deleted = 0 AND id <> ? ORDER BY id ASC', [$forum_id, $topic_id])->results();
...
$smarty->assign([
    ...
    'CANCEL_LINK' => URL::build('/forum/konu/' . urlencode($topic_id)),
    'TOPICS' => $topics
]);
