...
class LatestStorePurchasesWidget extends WidgetBase {
	...
	public function initialise(): void {
		...
		if ($this->_cache->isCached('latest_purchases')) {
			...
			$product_id  = DB::getInstance()->query('SELECT product_id FROM rw_store_payments')->results();
			$product_name= DB::getInstance()->query('SELECT name FROM rw_store_payments WHERE id $product_id ')->results();
