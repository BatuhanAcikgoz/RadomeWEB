...
class Haberler_Sitemap {
    ...
    public static function generateSitemap(Sitemap $sitemap): void {
        ...
        $topics = $db->query('SELECT id, haberler_id, topic_title FROM rw_topics WHERE deleted = 0 AND haberler_id IN (SELECT haberler_id FROM rw_haberlers_permissions WHERE group_id = 0 AND `view` = 1)')->results();
