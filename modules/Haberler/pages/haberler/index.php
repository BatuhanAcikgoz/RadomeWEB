...
$smarty->assign('FORUMS_TITLE', $haberler_language->get('haberler', 'haberlers'));
...
if ($cache->isCached('haberlers')) {
    ...
    if (count($haberlers)) {
        foreach ($haberlers as $key => $item) {
            $haberlers[$key]['link'] = URL::build('/haberler/bakis/' . urlencode($key) . '-' . $haberler->titleToURL($item['title']));
...
$smarty->assign('FORUMS', $haberlers);
$smarty->assign('YES', $language->get('general', 'yes'));
$smarty->assign('NO', $language->get('general', 'no'));
$smarty->assign('SUBFORUMS', $haberler_language->get('haberler', 'subhaberlers'));
