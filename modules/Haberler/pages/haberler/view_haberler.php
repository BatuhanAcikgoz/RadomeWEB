...
if ($haberler_query->redirect_haberler == 1) {
    ...
    if ($haberler->canViewOtherTopics($fid, $user_groups)) {
        $topics = DB::getInstance()->query('SELECT * FROM rw_topics WHERE haber_id = ? AND sticky = 0 AND deleted = 0 ORDER BY topic_reply_date DESC', [$fid])->results();
    } else {
        $topics = DB::getInstance()->query('SELECT * FROM rw_topics WHERE haber_id = ? AND sticky = 0 AND deleted = 0 AND topic_creator = ? ORDER BY topic_reply_date DESC', [$fid, $user_id])->results();
    }

    // Get sticky topics
    $stickies = DB::getInstance()->query('SELECT * FROM rw_topics WHERE haber_id = ? AND sticky = 1 AND deleted = 0 ORDER BY topic_reply_date DESC', [$fid])->results();
        ...
        foreach ($subhaberlers as $subhaberler) {
            ...
            if ($haberler->haberlerExist($subhaberler->id, $user_groups)) {
                if ($haberler->canViewOtherTopics($subhaberler->id, $user_groups)) {
                    $latest_post = DB::getInstance()->query('SELECT * FROM rw_topics WHERE haber_id = ? AND deleted = 0 ORDER BY topic_reply_date DESC', [$subhaberler->id])->results();
                } else {
                    $latest_post = DB::getInstance()->query('SELECT * FROM rw_topics WHERE haber_id = ? AND deleted = 0 AND topic_creator = ? ORDER BY topic_reply_date DESC', [$subhaberler->id, $user_id])->results();
                }
