        ...
        if (!isset($spam_check)) {
            ...
            if ($validate->passed()) {
                ...
                $content = EventHandler::executeEvent('preTopicCreate', [
                    ...
                    'alert_url' => URL::build('/haberler/haber/' . urlencode($id)),
...
if (count($users_following)) {
    ...
    foreach ($users_following as $user_following) {
                Alert::create(
                    $user_following->id,
                    'new_reply',
                    ['path' => ROOT_PATH . '/modules/Haberler/language', 'file' => 'haberler', 'term' => 'new_haber', 'replace' => ['{{author}}', '{{topic}}'], 'replace_with' => [Output::getClean($user->data()->nickname), Output::getClean($topic->haber_title)]],
                    ['path' => ROOT_PATH . '/modules/Haberler/language', 'file' => 'haberler', 'term' => 'new_haber', 'replace' => ['{{author}}', '{{topic}}'], 'replace_with' => [Output::getClean($user->data()->nickname), Output::getClean($topic->haber_title)]],
