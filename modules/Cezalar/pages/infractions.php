...
if(!file_exists(ROOT_PATH . '/modules/Cezalar/config.php')){
	die('Please configure the Cezalar module in the StaffCP first!');
}
require(ROOT_PATH . '/modules/Cezalar/config.php');
if(!isset($inf_db)) {
	die('Please configure the Cezalar module in the StaffCP first!');
}

require_once(ROOT_PATH . '/modules/Cezalar/classes/Cezalar.php');
switch($inf_config['plugin']) {
	case 'libertybans':
		// Libertybans integration
		require_once(ROOT_PATH . '/modules/Cezalar/classes/LibertyBans.php');
		$infractions = new LibertyBans($inf_db, $infractions_language);
	break;
	case 'litebans':
		// Litebans integration
		require_once(ROOT_PATH . '/modules/Cezalar/classes/LiteBans.php');
		$infractions = new LiteBans($inf_db, $infractions_language);
	break;
	case 'advancedban':
		// AdvancedBan integration
		require_once(ROOT_PATH . '/modules/Cezalar/classes/AdvancedBan.php');
...
if(!isset($_GET['view']) && !isset($_GET['id'])){
    $infractions_list = $infractions->listCezalar($p, 10);
            ...
            if(!isset($users_array[$result->name])){
                ...
                if($query_user->exists()){
                    ...
                    $users_array[$result->name] = array(
                        'profile' => null,
                        'style' => null,
                        'avatar' => AvatarSource::getAvatarFromUUID($result->name)
                    );
                }
            }
            if(!isset($users_array[$result->banned_by_name])){
                ...
                if($query_user->exists()){
                    ...
                    $users_array[$result->banned_by_name] = array(
                        'profile' => null,
                        'style' => null,
                        'avatar' => AvatarSource::getAvatarFromUUID($result->banned_by_name)
                    );
                }
            }
            if(isset($result->removed_by_name) && !isset($users_array[$result->removed_by_name])){
                ...
                if($query_user->exists()){
                    ...
                    $users_array[$result->removed_by_name] = array(
                        'profile' => null,
                        'style' => null,
                        'avatar' => AvatarSource::getAvatarFromUUID($result->removed_by_name)
                    );
                }
            }
