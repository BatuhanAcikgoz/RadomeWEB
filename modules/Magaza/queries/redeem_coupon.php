    ...
    if (Token::check()) {
        ...
        if ($validation->passed()) {
            if (!empty($_POST['coupon'])) {
                $coupon = DB::getInstance()->query('SELECT * FROM rw_store_coupons WHERE code = ? AND start_date < ? AND expire_date > ? ', [$_POST['coupon'], date('U'), date('U')]);
                if ($coupon->count()) {
                    $coupon = new Coupon($coupon->first()->id);
                    if ($coupon->exists()) {
                        // Check redeem limit
                        if ($coupon->data()->redeem_limit > 0) {
                            $limit = DB::getInstance()->query('SELECT DISTINCT(rw_store_orders.id) FROM rw_store_orders INNER JOIN rw_store_payments ON rw_store_payments.order_id=rw_store_orders.id WHERE coupon_id = ?', [$coupon->data()->id]);
                            if (count($limit->results()) >= $coupon->data()->redeem_limit) {
                                Session::flash('store_error', $store_language->get('general', 'redeem_limit_reached'));
                                Redirect::to(URL::build(Magaza::getMagazaPath() . '/checkout'));
                            }
                        }

                        // Check customer redeem limit
                        if ($coupon->data()->customer_limit > 0) {
                            $limit = DB::getInstance()->query('SELECT DISTINCT(rw_store_orders.id) FROM rw_store_orders INNER JOIN rw_store_payments ON rw_store_payments.order_id=rw_store_orders.id WHERE coupon_id = ? AND from_customer_id = ?', [$coupon->data()->id, $from_customer->data()->id]);
                        ...
                        if ($coupon->data()->min_basket > 0) {
                            if ($coupon->data()->min_basket > $shopping_cart->getTotalCents()) {
                                Session::flash('store_error', $store_language->get('general', 'redeem_min_basket_value', ['min_basket' => Magaza::fromCents($coupon->data()->min_basket)]));
                                Redirect::to(URL::build(Magaza::getMagazaPath() . '/checkout'));
                            }
                        }
