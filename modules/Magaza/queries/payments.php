...
$total = DB::getInstance()->query('SELECT COUNT(*) as `total` FROM rw_store_payments', [])->first()->total;

$query = 'SELECT rw_store_payments.*, order_id, rw_store_orders.user_id, to_customer_id FROM rw_store_payments LEFT JOIN rw_store_orders ON order_id=rw_store_orders.id';
$limit = '';
$order = '';
$params = [];
$where = '';
...
if (isset($_GET['search']) && $_GET['search']['value'] != '') {
    $customers_list = [0];
    $customers = DB::getInstance()->query('SELECT rw_store_customers.id FROM `rw_store_customers` LEFT JOIN rw_users ON rw_users.id=user_id WHERE rw_users.username LIKE ? OR rw_store_customers.username LIKE ?', ['%' . $_GET['search']['value'] . '%', '%' . $_GET['search']['value'] . '%']);
...
if (strlen($where) > 0) {
    $totalFiltered = DB::getInstance()->query('SELECT COUNT(*) as `total` FROM rw_store_payments LEFT JOIN rw_store_orders ON order_id=rw_store_orders.id' . $where, $params)->first()->total;
}
