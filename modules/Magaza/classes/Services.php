...
class Services {
    ...
    public function __construct() {
        $directories = glob(ROOT_PATH . '/modules/Magaza/services/*' , GLOB_ONLYDIR);
        foreach ($directories as $directory) {
            $folders = explode('/', $directory);

            if (file_exists(ROOT_PATH . '/modules/Magaza/services/' . $folders[count($folders) - 1] . '/Service.php')) {
                require_once(ROOT_PATH . '/modules/Magaza/services/' . $folders[count($folders) - 1] . '/Service.php');
