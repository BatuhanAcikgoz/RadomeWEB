...
class ShoppingCart extends Instanceable {
    ...
    public function __construct() {
        ...
        if (count($items)) {
            ...
            $products_query = DB::getInstance()->query('SELECT * FROM rw_store_products WHERE id in ('.$products_ids.') AND disabled = 0 AND deleted = 0 AND payment_type IN ('.$payment_type.')')->results();
            foreach ($products_query as $item) {
                $product = new Product(null, null, $item);

                EventHandler::executeEvent('renderMagazaProduct', [
                    'product' => $product,
