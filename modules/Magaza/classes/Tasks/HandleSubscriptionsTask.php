...
class HandleSubscriptionsTask extends Task {
    ...
    public function run(): string {
        $subscriptions_log = [];
        $subscriptions = DB::getInstance()->query('SELECT * FROM rw_store_subscriptions WHERE next_billing_date < ? AND expired = 0', [date('U')]);
    ...
    private function reschedule() {
        $hasBeenScheduled = DB::getInstance()->query('SELECT COUNT(*) c FROM rw_queue WHERE `task` = \'HandleSubscriptionsTask\' AND `status` = \'ready\'')->first()->c;
