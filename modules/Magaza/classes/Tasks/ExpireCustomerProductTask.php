...
class ExpireCustomerProductTask extends Task {
    ...
    public static function schedule(Order $order, Item $item, Payment $payment): void {
        $hasBeenScheduled = DB::getInstance()->query('SELECT COUNT(*) c FROM rw_queue WHERE `task` = \'ExpireCustomerProductTask\' AND entity_id = ?', [$item->getId()])->first()->c;
        ...
        if (!$hasBeenScheduled) {
            ...
            DB::getInstance()->query('UPDATE rw_store_orders_products SET expire = ?, task_id = ? WHERE id = ?', [$time, $task_id, $item->getId()]);
        }
    }
}