...
class ActionsHandler extends Instanceable {
    ...
    public function getActions(?Product $product = null, string $trigger = null): array {
        if ($product == null) {
            ...
            $actions = $this->_actions ??= (function (): array {
                ...
                $actions_query = DB::getInstance()->query('SELECT * FROM rw_store_products_actions WHERE product_id IS NULL ORDER BY `order` ASC');
            ...
            $actions = $this->_product_actions[$product->data()->id] ??= (function ($product): array {
                ...
                $actions_query = DB::getInstance()->query('SELECT * FROM rw_store_products_actions WHERE product_id = ? OR product_id IS NULL ORDER BY `order` ASC', [$product->data()->id]);
                if ($actions_query->count()) {
                    $services = Services::getInstance();
    ...
    public function getAction(int $id): ?Action {
        $actions_query = DB::getInstance()->query('SELECT * FROM rw_store_products_actions WHERE id = ?', [$id]);
        if ($actions_query->count()) {
