...
class PaymentInfoEndpoint extends KeyAuthEndpoint {
    ...
    public function execute(Radome2API $api): void {
        ...
        if (!$payment->exists()) {
            $api->throwError(MagazaApiErrors::ERROR_PAYMENT_NOT_FOUND);
        }
        ...
        $return = [
            ...
            'amount_cents' => (int) Magaza::toCents($payment->data()->amount ?? 0),
            'currency' => $payment->data()->currency,
            'fee' => $payment->data()->fee,
            'fee_cents' => (int) Magaza::toCents($payment->data()->fee ?? 0),
