...
class Minecraft_Community_Gateway extends GatewayBase {
    ...
    public function __construct() {
        ...
        $store_version = '1.7.1';
        $settings = ROOT_PATH . '/modules/Magaza/gateways/Minecraft-Community/gateway_settings/settings.php';
    ...
    public function processOrder(Order $order): void {
        ...
        $successRedirect = rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=Minecraft-Community&do=success');
        $cancelRedirect = rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=Minecraft-Community&do=cancel');

        $result = HttpClient::post('https://mccommunity.net/index.php?route=/api/v2/oauth2/store/order/create', json_encode([
            'client_id' => MagazaConfig::get('minecraft-community.client_id'),
            'client_secret' => MagazaConfig::get('minecraft-community.client_secret'),
            'order_id' => $order->data()->id,
            'currency' => $currency,
            'price' => Magaza::fromCents($order->getAmount()->getTotalCents()),
    ...
    public function handleListener(): void {
        ...
        if ($response->client_id != MagazaConfig::get('minecraft-community.client_id') || $response->client_secret != MagazaConfig::get('minecraft-community.client_secret')) {
            return;
        }
