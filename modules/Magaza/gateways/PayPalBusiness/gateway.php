...
class PayPal_Business_Gateway extends GatewayBase {
    ...
    public function __construct() {
        ...
        $store_version = '1.7.1';
        $settings = ROOT_PATH . '/modules/Magaza/gateways/PayPalBusiness/gateway_settings/settings.php';
    ...
    public function processOrder(Order $order): void {
        ...
        if (!$order->isSubscriptionMode()) {
            ...
            $amount->setTotal(Magaza::fromCents($order->getAmount()->getTotalCents()));
            ...
            $redirectUrls->setReturnUrl(rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=success'))
                ->setCancelUrl(rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=cancel'));
            ...
            if (isset($_GET['paymentId'])) {
                ...
                if (!$store_payment->exists()) {
                    ...
                    $store_payment->create([
                        ...
                        'amount_cents' => Magaza::toCents($transactions[0]->getAmount()->total),
                        'currency' => $transactions[0]->getAmount()->currency,
                        'fee_cents' => $sale->getTransactionFee() ? Magaza::toCents($sale->getTransactionFee()->getValue()) : null,
                        'created' => date('U'),
                        'last_updated' => date('U')
                    ]);
                }
                ...
                DB::getInstance()->insert('store_subscriptions', [
                    ...
                    'amount_cents' => Magaza::toCents($payment_definitions->getAmount()->getValue()),
    ...
    public function handleListener(): void {
        if (isset($_GET['key']) && $_GET['key'] == MagazaConfig::get('paypal_business.key')) {
            $this->getApiContext();
            ...
            $signatureVerification->setWebhookId(MagazaConfig::get('paypal_business.hook_key'));
                            ...
                            if (isset($response->resource->parent_payment)) {
                                ...
                                if ($payment->exists()) {
                                    ...
                                    $data = [
                                        ...
                                        'amount_cents' => Magaza::toCents($response->resource->amount->total),
                                        'currency' => $response->resource->amount->currency,
                                        'fee_cents' => Magaza::toCents($response->resource->transaction_fee->value ?? 0)
                                    ];
    ...
    private function getApiContext(): ?\PayPal\Rest\ApiContext {
        $client_id = MagazaConfig::get('paypal_business.client_id');
        $client_secret = MagazaConfig::get('paypal_business.client_secret');

        if ($client_id && $client_secret) {
            try {
                require_once(ROOT_PATH . '/modules/Magaza/gateways/PayPalBusiness/autoload.php');
                ...
                $hook_key = MagazaConfig::get('paypal_business.hook_key');
                if (!$hook_key) {
                    $key = md5(uniqid());
                    ...
                    $webhook->setUrl(rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/listener', 'gateway=PayPalBusiness&key=' . $key));
                    $webhook->setEventTypes($webhookEventTypes);
                    $output = $webhook->create($apiContext);
                    $id = $output->getId();

                    MagazaConfig::setMultiple([
    ...
    public function getPlan(Product $product): PayPal\Api\Plan {
        $plan_query = DB::getInstance()->query('SELECT * FROM rw_store_products_meta WHERE product_id = ? AND name = ?', [$product->data()->id, 'paypal_plan_id']);
        if ($plan_query->count()) {
            // Use existing plan
    ...
    public function createPlan(Product $product): PayPal\Api\Plan {
        ...
        $paymentDefinition->setName('Regular Payments')
            ...
            ->setAmount(new PayPal\Api\Currency(array('value' => Magaza::fromCents($product->data()->price_cents), 'currency' => Magaza::getCurrency())));

        $merchantPreferences = new PayPal\Api\MerchantPreferences();
        $merchantPreferences->setReturnUrl(rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=success'))
            ->setCancelUrl(rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=cancel'))
