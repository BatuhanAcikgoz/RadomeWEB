...
class PayPal_Business_Gateway extends GatewayBase {
    ...
    public function processOrder(Order $order): void {
        ...
        $redirectUrls->setReturnUrl(rtrim(Util::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=success'))
            ->setCancelUrl(rtrim(Util::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=PayPalBusiness&do=cancel'));
                ...
                if (isset($response->event_type)) {
                    ...
                    switch ($response->event_type) {
                        ...
                        case 'BILLING.SUBSCRIPTION.CANCELLED':
                            ...
                            DB::getInstance()->query('UPDATE `rw_store_agreements` SET status = ?, updated = ? WHERE agreement_id = ?', [
                                2,
                                date('U'),
                                $id
                            ]);
        ...
        if ($client_id && $client_secret) {
            try {
                ...
                if (!$hook_key) {
                    ...
                    $webhook->setUrl(rtrim(Util::getSelfURL(), '/') . URL::build('/magaza/listener', 'gateway=PayPalBusiness&key=' . $key));
                    $webhook->setEventTypes($webhookEventTypes);
                    $output = $webhook->create($apiContext);
                    $id = $output->getId();
                    
                    MagazaConfig::set(['paypal_business/key' => $key, 'paypal_business/hook_key' => $id]);
                }
