...
class Stripe_Gateway extends GatewayBase implements SupportSubscriptions {
    ...
    public function __construct() {
        ...
        $store_version = '1.7.1';
        $settings = ROOT_PATH . '/modules/Magaza/gateways/Stripe/gateway_settings/settings.php';
    ...
    public function processOrder(Order $order): void {
        ...
        $successRedirect = rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=Stripe&do=success');
        $cancelRedirect = rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/islem/', 'gateway=Stripe&do=cancel');
    ...
    public function handleListener(): void {
        ...
        $webhook_secret = MagazaConfig::get('stripe.hook_key');
        if (!$webhook_secret) {
    ...
    private function getApiContext(): ?\Stripe\StripeClient {
        $secret_key = MagazaConfig::get('stripe.secret_key');

        if ($secret_key) {
            try {
                require_once(ROOT_PATH . '/modules/Magaza/gateways/Stripe/vendor/autoload.php');
                $stripe = new \Stripe\StripeClient($secret_key);

                $hook_key = MagazaConfig::get('stripe.hook_key');
                if (!$hook_key) {
                    $webhook = $stripe->webhookEndpoints->create([
                        'url' => rtrim(URL::getSelfURL(), '/') . URL::build('/magaza/listener', 'gateway=Stripe'),
                        'enabled_events' => [
    ...
    public function updateWebhook(Stripe\StripeClient $stripe) {
        $hook_id = MagazaConfig::get('stripe.hook_id');
        $hook_key = MagazaConfig::get('stripe.hook_key');
        if ($hook_key && !$hook_id) {
            ...
            if ($last_id) {
                MagazaConfig::set('stripe.hook_id', $last_id);
            }
        }
    }
}
