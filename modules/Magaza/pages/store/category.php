...
// Query category
$category = DB::getInstance()->query('SELECT * FROM rw_store_categories WHERE id = ?', [$category_id]);
if (!$category->count()) {
    require_once(ROOT_PATH . '/404.php');
    die();
}
...
// Get products
$products = DB::getInstance()->query('SELECT id, name, `order`, price, description, image FROM rw_store_products WHERE category_id = ? AND disabled = 0 AND hidden = 0 AND deleted = 0 ORDER BY `order` ASC', [$category_id]);
