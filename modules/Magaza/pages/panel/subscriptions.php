 ...
 *  License: MIT
 *
 *  Magaza module - panel subscriptions page
 */
...
if (!isset($_GET['subscription'])) {
    ...
    $subscriptions_query = DB::getInstance()->query('SELECT * FROM rw_store_subscriptions ORDER BY id DESC');
    if ($subscriptions_query->count()) {
        ...
        foreach ($subscriptions_query->results() as $item) {
            ...
            if ($customer->exists() && $customer->getUser()->exists()) {
                ...
                $link = URL::build('/panel/kullanicilar/magaza/', 'user=' . $customer_user->data()->id);
            } else {
                $username = $customer->getUsername();
            ...
            $subscriptions_list[] = [
                ...
                'link' => URL::build('/panel/magaza/abonelikler/', 'subscription=' . $subscription->data()->id)
            ];
        }
    ...
    if (!$subscription->exists()) {
        Redirect::to(URL::build('/panel/magaza/abonelikler'));
    }
        ...
        if (Token::check()) {
            if (Input::get('action') == 'sync') {
                ...
                if ($subscription->sync()) {
                    ...
                    Redirect::to(URL::build('/panel/magaza/abonelikler/', 'subscription=' . $subscription->data()->id));
                } else {
                ...
                if ($subscription->cancel()) {
                    ...
                    Redirect::to(URL::build('/panel/magaza/abonelikler/', 'subscription=' . urlencode($subscription->data()->id)));
                } else {
    ...
    // Payments
    $payments_list = [];
    $payments_query = DB::getInstance()->query('SELECT * FROM rw_store_payments WHERE subscription_id = ? ORDER BY created DESC', [$subscription->data()->id]);
    if ($payments_query->count()) {
        foreach ($payments_query->results() as $payment) {
            $payments_list[] = [
                'date' => date(DATE_FORMAT, $payment->created),
                'link' => URL::build('/panel/magaza/odemeler', 'payment=' . $payment->id)
            ];
        }
    }
    ...
    $smarty->assign([
        ...
        'BACK_LINK' => URL::build('/panel/magaza/abonelikler'),
        ...
        'AMOUNT_VALUE' => Magaza::fromCents($subscription->data()->amount_cents),
