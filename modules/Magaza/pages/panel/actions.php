 ...
 *  License: MIT
 *
 *  Magaza module - panel actions page
 */
...
if (!isset($_GET['action'])) {
    ...
    foreach (ActionsHandler::getInstance()->getActions() as $action) {
        ...
        $actions_array[] = [
            ...
            'edit_link' => URL::build('/panel/magaza/eylemler', 'action=edit&aid=' . $action->data()->id),
            'delete_link' => URL::build('/panel/magaza/eylemler', 'action=delete&aid=' . $action->data()->id)
        ];
    }
    ...
    $smarty->assign([
        ...
        'NEW_ACTION_LINK' => URL::build('/panel/magaza/eylemler/' , 'action=new'),
    ...
    if (isset($_GET['product'])) {
        ...
        if (!$product->exists()) {
            Redirect::to(URL::build('/panel/magaza/eylemler'));
        }
    }
    ...
    switch ($_GET['action']) {
            ...
            if (!isset($_GET['service'])) {
                ...
                foreach ($services->getAll() as $service) {
                    $services_list[] = [
                        ...
                        'select_link' => URL::build('/panel/magaza/eylemler/' , 'action=new&service=' . $service->getId() . (isset($_GET['product']) ? '&product=' . $_GET['product'] : '')),
                    ];
                }
                ...
                $smarty->assign([
                    ...
                    'BACK_LINK' => $product == null ? URL::build('/panel/magaza/eylemler') : URL::build('/panel/magaza/urun/' , 'product=' . $product->data()->id),
        ...
        case 'delete';
            ...
            Redirect::to($product == null ? URL::build('/panel/magaza/eylemler') : URL::build('/panel/magaza/urun/' , 'product=' . $product->data()->id));
            break;
        default:
            Redirect::to(URL::build('/panel/magaza/eylemler'));
    }
