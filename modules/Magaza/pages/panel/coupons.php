 ...
 *  License: MIT
 *
 *  Magaza module - panel coupons page
 */
...
if (!isset($_GET['action'])) {
    ...
    $coupons = DB::getInstance()->query('SELECT * FROM rw_store_coupons ORDER BY `expire_date` DESC');
    if ($coupons->count()) {
        $coupons_list = [];
    ...
    switch ($_GET['action']) {
        case 'new';
                    ...
                    if ($validation->passed()) {
                        ...
                        try {
                            ...
                            DB::getInstance()->insert('store_coupons', [
                                ...
                                'min_basket' => Magaza::toCents(Input::get('min_basket')),
                            ]);
            ...
            $products_list = [];
            $products = DB::getInstance()->query('SELECT * FROM rw_store_products WHERE deleted = 0')->results();
        ...
        case 'edit';
            ...
            $smarty->assign([
                ...
                'MIN_BASKET' => $store_language->get('admin', 'minimum_basket'),
                'MIN_BASKET_VALUE' => Magaza::fromCents($coupon->min_basket),
            ]);
