    ...
    public static function globalLimit(array $params = []): array {
        ...
        if (isset($global_limit['limit']) && $global_limit['limit'] > 0) {
            ...
            if (isset($global_limit['period']) && $global_limit['period'] != 'no_period' && isset($global_limit['interval']) && $global_limit['interval'] > 0) {
                $limit = DB::getInstance()->query('SELECT DISTINCT(rw_store_orders_products.order_id) FROM rw_store_orders_products INNER JOIN rw_store_payments ON rw_store_payments.order_id=rw_store_orders_products.order_id WHERE product_id = ? AND created > ?', [$product->data()->id, strtotime('-'.$global_limit['interval'].' ' . $global_limit['period'])]);
            } else {
                $limit = DB::getInstance()->query('SELECT DISTINCT(rw_store_orders_products.order_id) FROM rw_store_orders_products INNER JOIN rw_store_payments ON rw_store_payments.order_id=rw_store_orders_products.order_id WHERE product_id = ?', [$product->data()->id]);
            }
