...
if (Input::exists()) {
    ...
    if (Token::check(Input::get('token'))) {
        ...
        $validation = Validate::check($_POST, [
            ...
            'requirePlayer' => [
                Validate::IN => 'Invalid requirePlayer'
            ]
        ]);
        ...
        if ($validation->passed()) {
            ...
            if (!$action->exists()) {
                // Create new action
                $last_order = DB::getInstance()->query('SELECT `order` FROM rw_store_products_actions ORDER BY `order` DESC LIMIT 1')->results();
                ...
                DB::getInstance()->insert('store_products_actions', [
                    ...
                    'require_online' => Input::get('requirePlayer'),
                    'order' => $last_order + 1,
                ...
                // Handle selected connections if its use own connection list
                if (!in_array(0, $selected_connections)) {
                    // Check for new connections to give action which they don't already have
                    foreach ($selected_connections as $connection) {
...
$smarty->assign([
    ...
    'SETTINGS_TEMPLATE' => ROOT_PATH . '/modules/Magaza/services/MinecraftServer/settings/action_settings.tpl'
]);